...
import { ArrowLeft, Check, CircleCheckBig, ClipboardList, Clock, MapPin, Search, ThumbsDown, ThumbsUp, TrafficCone, User, Wrench } from 'lucide-react';
...
export default function ShowComplaint() {
    ...
    const startComplaint = () => {
        router.patch(route('municipality.complaints.start', complaint.id));
    };

    const endComplaint = () => {
        router.patch(route('municipality.complaints.end', complaint.id));
    };
            ...
            <div className="border border-border rounded-xl bg-zinc-900 m-4 p-4">
                <div className="border border-border bg-primary-foreground rounded-xl p-4 w-full">
                    <div className="flex justify-between items-start mb-4">
                        <Link href={route('municipality.complaints.index')}>
                    ...
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="flex flex-row gap-2">
                            ...
                            {complaint?.status_id === 2 && (
                                <button onClick={() => endComplaint()} className={"w-fit cursor-pointer border border-green-500 bg-primary-foreground rounded-xl py-2 px-4 group flex items-center justify-center gap-2 font-semibold text-foreground hover:bg-green-500/60 transition-all duration-200"}>
                                    Encerrar Trabalhos
                                    <CircleCheckBig size={18} className="text-green-500 group-hover:text-foreground" />
                                </button>
                            )}
                        </div>
                    </div>
